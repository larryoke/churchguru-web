...
public class GiveViewImpl extends DetailViewGwtImpl implements GiveView {
	...
	private MCheckBox isConsent = new MCheckBox();
	...
	public GiveViewImpl() {
		...
		addPrivacyPanelPanel(formContainer);
	...
	public void addPayButton(FlowPanel container) {
		...
		paypalButton.addTapHandler(new TapHandler() {
			...
			public void onTap(TapEvent event) {
				...
				if (!isConsent.getValue()) {
					Dialogs.alert("Donor details", "Please review and consent to the Privacy Policy",
							new AlertCallback() {
								@Override
								public void onButtonPressed() {
								}
	...
	private void addPrivacyPanelPanel(FlowPanel container) {
		WidgetList privacyPanelPanel = new WidgetList();
		privacyPanelPanel.setHeader(new HeaderLabel("Privacy Policy"));

		isConsent.setTitle("is consent");
		isConsent.setValue(false);
		updateWidgetFromCookie(isConsent, "isConsent");
		isConsent.addTapHandler(new TapHandler() {
			@Override
			public void onTap(TapEvent event) {
				updateCookie(isConsent, "isConsent");
			}
		});
		...
		HTML html = new HTML(
				"Trinity Chapel uses the information collected from this mobile app in a manner that is consistent with the Privacy Policy.");
		html.setStylePrimaryName("normalLabel");
		vp.add(html);

		vp.add(new HTML("&nbsp;"));
		html = new HTML("By continuing, you agree to the terms and acknowledge our privacy policy.");
		html.setStylePrimaryName("normalLabel");
		vp.add(html);

		Anchor anchor = new Anchor("Privacy Policy");
		...
		privacyPanelPanel.add(outerPanel);
		container.add(privacyPanelPanel);
	}
