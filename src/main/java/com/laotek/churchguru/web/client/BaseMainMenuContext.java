...
public abstract class BaseMainMenuContext {
     ...
     * @return
     */
    protected boolean hasRoleRequired(UserRoleName requiredUserRoleName,
                                      UserRoleName currentUserRoleName) {
        if (requiredUserRoleName.equals(currentUserRoleName)) {
            return true;
        } else if (currentUserRoleName != null
                && currentUserRoleName.getChildren() != null
    ...
    protected void createLink(Image image, FlexTable panel, final Anchor anchor) {
        ...
        panel.getCellFormatter().setVerticalAlignment(row, 1,
                HasVerticalAlignment.ALIGN_TOP);
    }

    protected void createLink(Image image, FlexTable panel,
                              final Anchor anchor, UserRoleName requiredUserRoleName,
                              UserRoleName currentUserRoleName) {
    ...
    protected void createLink(Image image, String label, boolean blinkAnchor,
                              FlexTable panel, final Place place,
                              UserRoleName minimumUserRoleName, UserRoleName currentUserRoleName) {
        Anchor link = new Anchor(label);
        if (blinkAnchor) {
            blinkingAnchors.add(link);
        }
        createLink(image, link, panel, place, minimumUserRoleName,
                currentUserRoleName);
    }

    private void createLink(Image image, Anchor link, FlexTable panel,
                            final Place place, UserRoleName minimumUserRoleName,
                            UserRoleName currentUserRoleName) {
        int row = panel.getRowCount();
        image.setWidth("30px");
        panel.setWidget(row, 0, image);
    ...
    protected void createPostNoticeOrEventLink(Image image, FlexTable panel,
                                               UserRoleName minimumUserRoleName, UserRoleName currentUserRoleName) {
        int row = panel.getRowCount();
        image.setWidth("30px");
        panel.setWidget(row, 0, image);
        ...
        link.addClickHandler(new ClickHandler() {
            @Override
            public void onClick(ClickEvent event) {
                String subject = Window
                        .prompt("Please start here by submitting the subject of new notice or event",
                                "");
                if (subject != null && !subject.trim().equals("")) {
                    NoticeAndEventAction action = new NoticeAndEventAction(
                            subject, EnumNoticeOrEventAction.CREATE);
    ...
    protected void createNewListeningMessageLink(Image image, FlexTable panel,
                                               UserRoleName minimumUserRoleName, UserRoleName currentUserRoleName) {
        int row = panel.getRowCount();
        image.setWidth("30px");
        panel.setWidget(row, 0, image);
        ...
        link.addClickHandler(new ClickHandler() {
            ...
            public void onClick(ClickEvent event) {
                String subject = Window
                        .prompt("Please provide the title of the new audio message to be uploaded",
                                "");
                if (subject != null && !subject.trim().equals("")) {

                    CreateNewMessageAction action = new CreateNewMessageAction(subject);
                    UserContext.getInstance().decorateClientSessionId(action);
    ...
    protected void createNewWatchingMessageLink(Image image, FlexTable panel,
                                               UserRoleName minimumUserRoleName, UserRoleName currentUserRoleName) {
        int row = panel.getRowCount();
        image.setWidth("30px");
        panel.setWidget(row, 0, image);
        ...
        link.addClickHandler(new ClickHandler() {
            ...
            public void onClick(ClickEvent event) {
                String subject = Window
                        .prompt("Please provide the title of the new video message to be uploaded",
                                "");
                if (subject != null && !subject.trim().equals("")) {
                    CreateNewMessageAction action = new CreateNewMessageAction(subject);
                    UserContext.getInstance().decorateClientSessionId(action);
    ...
    protected void createHelpLink(Image image, Anchor link, FlexTable panel,
        ...
        link.addClickHandler(new ClickHandler() {
            ...
            public void onClick(ClickEvent event) {
                ...
                UrlBuilder builder = Window.Location.createUrlBuilder()
                        .setProtocol(protocol).setHost(host).setPath(placeName);
    ...
    protected Anchor createHelpAnchor(final String url) {
        ...
        helpAnchor.addClickHandler(new ClickHandler() {
            ...
            public void onClick(ClickEvent event) {
                ...
                String host = Window.Location.getHost();
                UrlBuilder builder = Window.Location.createUrlBuilder()
                        .setProtocol(protocol).setHost(host).setPath(url)
                        .setHash(null);
                String url = builder.buildString();
                Window.open(url, "_blank", "");
