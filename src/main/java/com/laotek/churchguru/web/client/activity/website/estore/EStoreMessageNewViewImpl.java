...
public class EStoreMessageNewViewImpl extends BaseViewImpl implements EStoreMessageNewView {
    ...
    public EStoreMessageNewViewImpl() {
    ...
    @Override
    public void initNewMessage(EStoreMessageDto dto, List<EStoreSpeakerDto> speakers,
	    List<EStoreCategoryDto> categories, List<EStoreMessagePictureDto> pictures,
	...
	EStoreSpeakerDto sdto = dto.getSpeakerDto();
	for (EStoreSpeakerDto speaker : speakers) {
	...
	EStoreCategoryDto cdto = dto.getCategoryDto();
	for (EStoreCategoryDto category : categories) {
	    if (cdto != null) {
	...
	EStoreMessagePictureDto pictureDto = dto.getPictureDto();
	for (EStoreMessagePictureDto picture : pictures) {
	    if (pictureDto != null) {
	...
	List<EStoreNotificationDto> messageNotifications = dto.getNotificationDtos();
	if (messageNotifications != null && messageNotifications.size() > 0) {
	    for (EStoreNotificationDto ndto : messageNotifications) {
		if (ndto.getNotificationType().equals(EStoreNotificationType.NOTIFY_ALL_FULL_MEMBERS)) {
		    notifyFullMembers.getCheckbox().setValue(true);
		}

		if (ndto.getNotificationType().equals(EStoreNotificationType.NOTIFY_GUESTS)) {
		    notifyGuests.getCheckbox().setValue(true);
		}

		if (ndto.getNotificationType().equals(EStoreNotificationType.NOTIFY_NON_WORKERS)) {
		    notifyNonWorkers.getCheckbox().setValue(true);
		}

		if (ndto.getNotificationType().equals(EStoreNotificationType.NOTIFY_ONLINE_MEMBERS)) {
		    notifyOnlineMembers.getCheckbox().setValue(true);
		}

		if (ndto.getNotificationType().equals(EStoreNotificationType.NOTIFY_WORKERS)) {
	...
	upload.addClickHandler(new ClickHandler() {
		...
		if (messageDatePicker.getValue() == null) {
		    ...
		    SubmitEStoreMessageAction action = new SubmitEStoreMessageAction();
		    ...
		    List<EStoreNotificationType> notifications = new ArrayList<EStoreNotificationType>();
		    if (notifyOnlineMembers.getCheckbox().getValue()) {
			notifications.add(EStoreNotificationType.NOTIFY_ONLINE_MEMBERS);
		    }
		    if (notifyFullMembers.getCheckbox().getValue()) {
			notifications.add(EStoreNotificationType.NOTIFY_ALL_FULL_MEMBERS);
		    }
		    if (notifyGuests.getCheckbox().getValue()) {
			notifications.add(EStoreNotificationType.NOTIFY_GUESTS);
		    }
		    if (notifyWorkers.getCheckbox().getValue()) {
			notifications.add(EStoreNotificationType.NOTIFY_WORKERS);
		    }
		    if (notifyNonWorkers.getCheckbox().getValue()) {
			notifications.add(EStoreNotificationType.NOTIFY_NON_WORKERS);
		    }
