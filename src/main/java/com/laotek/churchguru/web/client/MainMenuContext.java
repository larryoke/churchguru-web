...
public class MainMenuContext extends BaseMainMenuContext {
	...
	createInstantMessagePanel(stackMenu);
	...
	estorePanel.setHeight("100%");
	UserDto dto = UserContext.getInstance().getUserDto();
	initStackLayoutPanel(estorePanel, "images/app/cart.png", "Manage E-Store", stackMenu,
		new EStoreMessagesPlace("messages"), UserRoleName.ORGANISATION_DATA_VIEW_ONLY,
		dto.getOrganisationRole());

	createLink(new Image("images/app/cart.png"), "E-Store", false, estorePanel, new EStoreMessagesPlace("messages"),
		UserRoleName.ORGANISATION_DATA_VIEW_ONLY, dto.getOrganisationRole());
    }

    private void createInstantMessagePanel(StackLayoutPanel stackMenu) {
    ...
    public void showEstorePanel(String anchorLabel) {
	stackMenu.showWidget(estorePanel);
	highlightSelection(estorePanel, anchorLabel);
    }
